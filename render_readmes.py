...
def render_readme(tpl_path: Path, pythons: List[str], images=None):
    env = Environment(
    ...
    out_path.write_text(
        template.render(
            pythons=pythons,
            image_dir=tpl_path.parent.relative_to(THIS_DIR),
            images=images,
        )
    )
...
if __name__ == "__main__":
    ...
    for tpl in THIS_DIR.glob("**/image_readme.jinja"):
        ...
        images.append(tpl.relative_to(THIS_DIR).parent)
    render_readme(Path("main_readme.jinja"), pythons, images=images)
